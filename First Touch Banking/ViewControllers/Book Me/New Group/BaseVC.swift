...
//        print(arr)
    ...
    public func getMiniStatement(completionHandler: @escaping (_ result:MiniStatement) -> Void) {
        ...
        NetworkManager.sharedInstance.sessionManager?.request(compelteUrl, method: .post, parameters: params , encoding: URLEncoding.httpBody, headers:header).responseObject { (response: DataResponse<MiniStatement>) in
