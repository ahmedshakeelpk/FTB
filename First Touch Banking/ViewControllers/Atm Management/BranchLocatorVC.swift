...
class BranchLocatorVC: BaseVC , UITableViewDelegate , UITableViewDataSource ,UISearchResultsUpdating {
    
    
    @IBOutlet var branchesTableView: UITableView!
    var branchesObj : BranchLocator?
    var filteredObjects = [SingleBranchLocator]()
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        lblMain.text = "Branch Locator".addLocalizableString(languageCode: languageCode)
        self.branchesTableView.rowHeight = UITableViewAutomaticDimension
        self.branchesTableView.estimatedRowHeight = 130.0
        self.getBranches()
    ...
    func updateResults(){
        ...
        self.filteredObjects = (self.branchesObj?.branchLocators)!
        ...
        self.branchesTableView.tableHeaderView = searchController.searchBar
    ...
    func updateSearchResults(for searchController: UISearchController) {
        ...
        if searchController.searchBar.text == ""{
            ...
            self.filteredObjects = (self.branchesObj?.branchLocators?.filter {
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        ...
        //        if let count = self.branchesObj?.branchLocators?.count {
        //            return count
        //        }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let aCell = tableView.dequeueReusableCell(withIdentifier: "BranchLocatorTableViewCell") as! BranchLocatorTableViewCell
        aCell.selectionStyle = .none
        
        //        let aBranch = self.branchesObj?.branchLocators![indexPath.row]
        
        aCell.lblBranchTitle.text = self.filteredObjects[indexPath.row].name
        aCell.lblBranchAddress.text = self.filteredObjects[indexPath.row].address
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        let aValue:SingleBranchLocator = (self.branchesObj?.branchLocators![indexPath.row])!
...
//        self.rootVC?.present(detailsVC, animated: true, completion: nil)
    ...
    private func getBranches() {
        ...
        let compelteUrl = Constants.BASE_URL + "api/v1/Locations/Branch"
        ...
        NetworkManager.sharedInstance.sessionManager?.request(compelteUrl, method: .get, encoding: URLEncoding.httpBody, headers:header).response { response in
            //        Object { (response: DataResponse<BranchLocator>) in
            //        Alamofire.request(compelteUrl, headers:header).responseObject { (response: DataResponse<BranchLocator>) in
            ...
            if let json = try? JSONSerialization.jsonObject(with: data, options: []) {
                ...
                if response.response?.statusCode == 200 {
                    self.branchesObj = Mapper<BranchLocator>().map(JSONObject: json)
                    self.updateResults()
                    //   self.branchesObj = response.result.value
                    self.branchesTableView.reloadData()
                }
                else {
